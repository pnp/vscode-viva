...
export class SharePointFrameworkProjectUpgrade implements LanguageModelTool<ISharePointFrameworkProjectUpgrade> {
    async invoke(
        ...
        const response = `To perform the upgrade follow the following steps: ${result.stdout}. You should always build the project after the upgrade to ensure everything is working correctly and check for errors.`;
