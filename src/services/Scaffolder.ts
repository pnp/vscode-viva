...
export class Scaffolder {
	public static registerCommands() {
		...
		subscriptions.push(
			commands.registerCommand(
				...
				Scaffolder.showCreateProjectForm
			)
		);
		subscriptions.push(
			commands.registerCommand(
				Commands.addToProject,
				Scaffolder.showAddProjectForm
			)
		);
	}
	...
	public static async addComponentToProject(
		input: SpfxAddComponentCommandInput
	) {
	...
	public static async useSample(sample: Sample) {
		...
		if (!folderPath) {
			Notifications.warning(
				"You must select the parent folder to create the project in"
			);
			return;
		}
		...
		await window.withProgress(
			...
			async (progress) => {
				...
				const result = await CliExecuter.execute("spfx project rename", "md", {
					newName: projectName,
					generateNewId: true,
				});
				...
				Scaffolder.createProjectFileAndOpen(
					solutionPath,
					isScenario ? ProjectFileContent.initScenario : ProjectFileContent.init
				);
			}
		);
	}
	...
	public static async pickFolder() {
		...
		if (folder?.length) {
			PnPWebview.postMessage(
				WebviewCommand.toWebview.folderPath,
				folder[0].fsPath
			);
		}
	}
	 ...
	 */
	public static validateSolutionName(
		folderPath: string,
		solutionNameInput: string
	) {
		if (existsSync(join(folderPath, solutionNameInput))) {
			PnPWebview.postMessage(
				WebviewCommand.toWebview.validateSolutionName,
				false
			);
			return;
		}
	 ...
	 */
	public static async validateComponentName(
		componentType: ComponentType,
		componentNameInput: string
	) {
		if (
			await Scaffolder.componentFolderExists(componentType, componentNameInput)
		) {
			PnPWebview.postMessage(
				WebviewCommand.toWebview.validateComponentName,
				false
			);
			return;
		}
	...
	/**
	 * Returns the value of the createNVMRCFileDefaultValue setting and sends it to the webview.
	 */
	 ...
	 */
	private static async scaffold(
		input: SpfxScaffoldCommandInput | SpfxAddComponentCommandInput,
		...
		const yoCommandSolutionName = isNewProject
			? ` --solution-name "${(input as SpfxScaffoldCommandInput).solutionName}"`
			: "";
		...
		await window.withProgress(
			...
			async () => {
				try {
					let folderPath = isNewProject
						? (input as SpfxScaffoldCommandInput).folderPath
						: "";
					if (!folderPath) {
					...
					if (result !== 0) {
						Notifications.errorNoLog(
							`Error creating the component. Check [output window](command:${Commands.showOutputChannel}) for more details.`
						);
						return;
					}
					...
					if (isNewProject) {
						...
						const newFolderPath = join(
							newSolutionInput.folderPath,
							newSolutionInput.solutionName!
						);

						let content = newSolutionInput.shouldRunInit
							? ProjectFileContent.init
							: "";
	...
	private static async getFolderPath(): Promise<string | undefined> {
		...
		const folderPath = await window
			.showQuickPick(folderOptions, {
				canPickMany: false,
				ignoreFocusOut: true,
	 ...
	 */
	private static async createProjectFileAndOpen(
		folderPath: string,
		content: any
	) {
		if (content) {
			writeFileSync(join(folderPath, PROJECT_FILE), content, {
				encoding: "utf8",
			});
		}
		...
		if (getPlatform() === "windows") {
			await commands.executeCommand(
				"vscode.openFolder",
				Uri.file(parseWinPath(folderPath))
			);
		} else {
	 ...
	 */
	private static async getSolutionName(
		folderPath: string
	 ...
	 */
	private static async componentFolderExists(
		type: ComponentType,
		value: string
	) {
		let componentFolder = "";
